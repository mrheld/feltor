...
int main(int argc, char * argv[])
{
	...
	//Elliptic
        ...
        [](double x) { return alpha*x*exp(-alpha*x);},
        [](double x) { return pow(alpha*x,2)/tgamma(2+1)*exp(-alpha*x);},
        [](double x) { return pow(alpha*x,3)/tgamma(3+1)*exp(-alpha*x);},
        [](double x) { return pow(alpha*x,4)/tgamma(4+1)*exp(-alpha*x);},
        [](double x) { return pow(alpha*x,5)/tgamma(5+1)*exp(-alpha*x);},
        [](double x) { return pow(alpha*x,10)/tgamma(10+1)*exp(-alpha*x);},
	//Helmholtz
	...
	if (u==8) dg::blas1::scal(x_exac, funcs[0](ell_fac)); //use exact exp(-alpha A) for GL
